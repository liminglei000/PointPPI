...
def sample_and_group_uniform(npoint, n_neighborhood, xyz, points):
    B, N, C = xyz.shape
    center_idx = uniform_center_sample(xyz, npoint)  # Replace `farthest_point_sample` with uniform sampling
    new_xyz = index_points(xyz, center_idx)
    idx = fixed_neighborhood_points(center_idx, xyz, n_neighborhood)  # Replace `query_ball_point`
