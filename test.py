...
def test(model, graph, test_mask, device, gnn_model="./"):
    ...
    for step in tqdm(range(valid_steps)):
        ...
        output, vectors = model(graph.x, graph.edge_index, valid_edge_id)
        ...
        for vector in range(1690):
            vector_list.append(vectors[vector].cpu().data)

    valid_predict_list = torch.cat(valid_predict_list, dim=0)
    valid_pre_result_list = torch.cat(valid_pre_result_list, dim=0)
    valid_label_list = torch.cat(valid_label_list, dim=0)
    # vector_list_front = torch.cat(vector_list, dim=0).reshape(1690, 512)
    vector_list_front = torch.stack(vector_list, dim=0)
    ...
    np.savetxt("vector_list_front.txt", vector_list_front, fmt="%.6f", delimiter=",")
...
def main():
    ...
    graph.train_mask = [i for i in range(0, 7624)]
    graph.val_mask = [i for i in range(0, 7624)]
